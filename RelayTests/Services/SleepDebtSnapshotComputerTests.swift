...
final class SleepDebtSnapshotComputerTests: XCTestCase {
    ...
    func test_snapshot_returnsNilBalances_whenStoreIsEmpty() {
        ...
        XCTAssertNil(snapshot.personABalance)
        XCTAssertNil(snapshot.personBBalance)
    ...
    func test_snapshot_computesNegativeBalance_forUnderSleptPerson() throws {
        ...
        let d = try store.startSession(for: .personA, at: referenceDate.addingTimeInterval(-5 * hour))
        try store.endSession(d, at: referenceDate.addingTimeInterval(-1 * hour))

        let snapshot = SleepDebtSnapshotComputer.snapshot(from: store, at: referenceDate)

        XCTAssertEqual(snapshot.personABalance ?? .infinity, -4 * hour, accuracy: 1.0)
        XCTAssertEqual(snapshot.personBBalance ?? .infinity, -8 * hour, accuracy: 1.0)
    }

    func test_snapshot_computesPositiveBalance_forOverSleptPerson() throws {
        // Bethany: 9h in the last 24h → balance = +1h.
        let b = try store.startSession(for: .personB, at: referenceDate.addingTimeInterval(-10 * hour))
        try store.endSession(b, at: referenceDate.addingTimeInterval(-1 * hour))

        let snapshot = SleepDebtSnapshotComputer.snapshot(from: store, at: referenceDate)

        XCTAssertEqual(snapshot.personBBalance ?? .infinity, 1 * hour, accuracy: 1.0)
    }
    ...
    func test_snapshot_countsOpenSessionElapsedTime_atReferenceDate() throws {
        ...
        _ = try store.startSession(for: .personA, at: referenceDate.addingTimeInterval(-2 * hour))

        let snapshot = SleepDebtSnapshotComputer.snapshot(from: store, at: referenceDate)

        XCTAssertEqual(snapshot.personABalance ?? .infinity, 2 * hour - 8 * hour, accuracy: 1.0)
    }
    ...
    func test_snapshot_projectsForward_byClippingTo24hWindowAtFutureDate() throws {
        ...
        let d = try store.startSession(for: .personA, at: referenceDate.addingTimeInterval(-8 * hour))
        ...
        XCTAssertEqual(nowSnap.personABalance ?? .infinity, 0, accuracy: 1.0)
        XCTAssertEqual(futureSnap.personABalance ?? .infinity, -8 * hour, accuracy: 1.0)
    }
}
